...
public class Cliente extends Usuario{
  ...
  // El Usuario Si o Si debe de registrar su nombre y rut para poder identificarlo
    public Cliente(String nombre, String rut, Date fechaDeNacimiento, int puntos, int pasos, String nombreUsuario, String clave, int rol){
    ...
    public Cliente(String nombre, String rut, Date fecha, String nombreUsuario, String clave, int rol) {
        super(nombreUsuario, clave, 1);
        this.nombre = nombre;
        this.rut = rut;
        this.fechaDeNacimiento = fecha;
    }
    public Cliente(String nombre, String rut, String nombreUsuario, String clave, int rol){
        super(nombreUsuario, clave, 1);
        this.nombre = nombre;
        this.rut = rut;
    }
  
    public Cliente(String nombre, String rut, Date fecha, int puntos, String nombreUsuario, String clave, int rol){
        super(nombreUsuario, clave, 1);
        this.nombre = nombre;
        this.rut = rut;
        this.fechaDeNacimiento = fecha;
 ...
 public void setNombre(String nombre){
  ...
  public void crearUsuario(){
    ...
    public String getDatosString(){
        ...
        String fecha = formatter.format(this.fechaDeNacimiento);
        String idMisiones = "";
        String idRecompensas = "";
        
        //Comprimir misiones
        for(int i = 0; i < this.misiones.length; i++){
            if(this.misiones[i] != null)
                idMisiones = idMisiones+this.misiones[i].getIdMision();
            else
                idMisiones = idMisiones+"-1";
            if(i < this.misiones.length - 1){
                idMisiones = idMisiones+";";
            }
            //System.out.println(i+"   "+idMisiones);
        }

        //Comprimir Recompensas
        for(int i = 0; i < this.recompensas.length; i++){
            if(this.recompensas[i] != null)
                idRecompensas = idRecompensas+this.recompensas[i].getIdRecompensa();
            else
                idRecompensas = idRecompensas+"-1";
            if(i < this.recompensas.length - 1){
                idRecompensas = idRecompensas+";";
            }
            //System.out.println(i+"   "+idRecompensas);
        }
        
       return(this.getNombreUsuario()+";"+this.getClave()+";"+this.getRol()+";"+this.nombre+";"+this.rut+";"+fecha+";"+this.puntos+";"+this.pasos+";"+idMisiones+";"+idRecompensas); 
    } 
